...
import org.apache.solr.search.facet.FacetFieldProcessor.ParallelAcc;
...
public class RelatednessAgg extends AggValueSource {
  ...
  public SlotAcc createSlotAcc(FacetContext fcontext, int numDocs, int numSlots) throws IOException {
    ...
    if (numSlots > 1 && fcontext.processor instanceof FacetFieldProcessorByArray &&
      ...
      // Parallel counts are only relevant where relatedness is required for primary sort, and thus must be calculated
      ...
      // never be >1, so combination with ParallelSKGSlotAcc would be meaningless, and is not supported.
      //
      // FacetFieldProcessorByHashDV *is* used in high-cardinality field contexts; however, it is only beneficial
      // when *domain* cardinality is low. Because parallel collection effectively induces a composite domain that is
      // a union with the background set (which is normally high-cardinality), combination with ParallelSKGSlotAcc
      // would in most cases be an antipattern, and is not currently supported.

      return new ParallelSKGSlotAcc(min_pop, fcontext, numSlots, fgSet.size(), bgSet.size(),
          ffp.getParallelCountAcc(new QueryResultKey(null, fgFilters, null, 0), fgSet, numSlots),
          ffp.getParallelCountAcc(new QueryResultKey(null, Collections.singletonList(bgQ), null, 0), bgSet, numSlots));
    } else {
  ...
  private static final class ParallelSKGSlotAcc extends SlotAcc implements ParallelAcc {
    ...
    public ParallelSKGSlotAcc(double minPopularity, FacetContext fcontext, int numSlots, int fgSize, int bgSize, CountSlotAcc fgCount, CountSlotAcc bgCount) {
      super(fcontext);
  ...
  private static final class SKGSlotAcc extends SlotAcc {
    ...
    // All these methods can/should be removed if/when parallel facet count collection is adopted (recommended)
